...
| 18 — Executive Summary    | Agentic reasoning instruction     | `step-18-executive-summary.json` |
...
**The pipeline is not considered complete until the Self-Audit (Step 17) passes.**

### Procedure

1. After Steps 10–17 complete, the orchestrator reads `step-17-audit.json`.
2. `overall_audit_result == "pass"` → Pipeline is finalized.
