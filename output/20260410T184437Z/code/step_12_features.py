...
def main() -> int:
    ...
    try:
        ...
        for window in tqdm(top_lags, desc="step12: rolling", unit="window"):
            ...
            lag_exprs.append(pl.col(target).rolling_mean(window_size=window).alias(mean_name))
            lag_exprs.append(pl.col(target).rolling_std(window_size=window).alias(std_name))
